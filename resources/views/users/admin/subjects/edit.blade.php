<x-app.admin.main-container>
    ...
    <x-card class="px-6 py-5 flex flex-col gap-y-5">
        <div class="card-title">
            <h1 class="font-bold text-2xl">Subject - Edit</h1>
        </div>
        <form action="{{ route('admin.subjects.update', $sub->id) }}" method="post" class="flex flex-col gap-y-4">
            @csrf
            @method('PATCH')
            ...
            <div>
                ...
                <x-form.text-input id="name" class="block mt-1 w-full" type="text" name="name"
                    value="{{ old('name', $sub->name) }}" required autocomplete="current-password"
