<x-app.admin.main-container>
    ...
    <x-card class="px-6 py-5 flex flex-col gap-y-5">
        <div class="card-title">
            <h1 class="font-bold text-2xl">Strand - Edit</h1>
        </div>
        <form action="{{ route('admin.strands.update', $strand->id) }}" method="post" class="flex flex-col gap-y-4">
            @csrf
            @method('PATCH')
            ...
            <div>
                ...
                <x-form.text-input id="name" class="block mt-1 w-full" type="text" name="name"
                    value="{{ old('name', $strand->name) }}" required autocomplete="current-password"
            ...
            <div>
                ...
                <x-form.text-input id="abbr" class="block mt-1 w-full" type="text" name="abbr"
                    value="{{ old('abbr', $strand->abbr) }}" required autocomplete="current-password"
